CREATE TEMPORARY TABLE Averages (subject_id bigint, average_mark int);
INSERT INTO Averages (subject_id, average_mark) SElECT subject_id, AVG(mark) FROM Mark GROUP BY student_id;
SELECT * FROM Subject WHERE id = (SELECT subject_id FROM Averages WHERE average_mark = (SELECT MAX(average_mark) FROM Averages));
CREATE TEMPORARY TABLE Totals (student_id bigint, total_paid decimal);
